    ...
    if moje_tipy.empty:
        ...
        for _, r in zobrazit.sort_values("kolo", ascending=False).iterrows():
            with st.container(border=True):
                ...
                tip_txt = f"Tvůj tip: {int(r['skore_domaci'])}:{int(r['skore_hoste'])}, xG {r['tip_xg']:.2f}"
                if pd.notna(r.get("skore_domaci_real", pd.NA)):
                    ...
                    st.markdown(f"{popis}  \n{tip_txt}  \nSkutečnost: {int(r['skore_domaci_real'])}:{int(r['skore_hoste_real'])}, xG {r['xg_slavia']:.2f}")
                    m1, m2, m3 = st.columns(3)
                    m1.metric("Za výsledek", f"{body.za_vysledek} b")
                    m2.metric("Za xG", f"{body.za_xg} b", f"odchylka {body.odchylka_xg}")
...
with tab_zebricek:
    if tipy_df.empty or vysledky_df.empty:
        ...
        souhrn["prumer_na_zapas"] = (souhrn["celkem_bodu"] / souhrn["odehrano"]).round(1)
